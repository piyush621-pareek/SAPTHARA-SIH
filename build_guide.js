...
const phases = [
  ...
  ["Phase 13 — Field-usability + delivery", "Make it real-user ready + self-updating.", "Self-service SIGN-UP with Driver/Field-Officer role selection (accounts stored in Postgres, bcrypt); From/To ROUTE PLANNER (pick origin + destination from NER places -> safe route + distance on demand); IN-APP AUTO-UPDATE (backend /app/version + APK hosting -> the app prompts 'Update available -> UPDATE', no Play Store, no re-send); PLACE NAMES instead of raw lat/lng everywhere; and field-bug fixes: report location via place-picker + real GPS button, a real Android location-permission request, and real device battery. Whole project recovered onto the D: drive under git version control."],
];
